...
for iProd in prodList :
  ...
  if 'iihe' in os.uname()[1]:
    ...
    if options.iStep == 'Prod' :
      ...
      fileCmd = 'ls '+eosTargBaseIn+'/'+iProd+'/'+options.iStep
    print 'hh ' , fileCmd , ' ' , prodDir, ' ' , Productions[iProd]['dirExt'] 
  else:
    if options.iStep == 'Prod' : 
    ...
    if ( not Productions[iProd]['isData'] and Steps[iStep]['do4MC'] ) or ( Productions[iProd]['isData'] and Steps[iStep]['do4Data'] ) :
      ...
      if 'iihe' in os.uname()[1]:
        ...
        if options.iStep == 'Prod' :
          ...
          fileCmd = 'ls '+eosTargBaseOut+'/'+iProd+'/'+options.iStep+'__'+iStep
      else:
        if options.iStep == 'Prod' :
                ...
                if aSample.replace('_25ns','') == iSample.replace('_25ns','') :
                  if 'iihe' in os.uname()[1]:
                    ...
                    if options.iStep == 'Prod' :
                      ...
                      targetList[iKey] = eosTargBaseIn+'/'+iProd+'/'+options.iStep+'/'+iFile
                  else:
                    if options.iStep == 'Prod' :
              ...
              if not iKey in targetListBaseW.keys():
                ...
                if 'iihe' in os.uname()[1]:
                  ...
                  if options.iStep == 'Prod' :
                    ...
                    targetListBaseW[iKey] = eosTargBaseIn+'/'+iProd+'/'+options.iStep+'/'+iFile
                else: 
                  if options.iStep == 'Prod' :
...
#              fileCmd = 'ls /pnfs/iihe/cms/store/user/xjanssen/HWW2015/'+iProd+'/'+PrevStep
            ...
            if '__' in options.iStep :
              ...
              for i in range(len(SubSteps)-1) :
                 ...
                 if len(SubSteps)-1 > 1 and i < len(SubSteps)-2 : PrevStep+='__'
#            if not '__' in  options.iStep :
            fileCmd = 'ls '+prodDir+Productions[iProd]['dirExt']
#            else:
#              fileCmd = 'ls /pnfs/iihe/cms/store/user/xjanssen/HWW2015/'+iProd+'/'+PrevStep
          else:
#            if not '__' in  options.iStep :
...
#              fileCmd = '/afs/cern.ch/project/eos/installation/'+aquamarineLocationIn+'/bin/eos.select ls '+eosTargBaseIn+'/'+iProd+'/'+PrevStep
      ...
      for iTarget in targetList.keys(): 
        ...
        if iStep == 'hadd' :
          ...
          outTree = '/gpfs/csic_projects/cms/'+os.getlogin()+'/latino_'+iTarget+'__'+iStep+'.root'
        ...
        # Chains of subTargets
          ...
          for iSubStep in  Steps[iStep]['subTargets'] :
            ...
            if selectSample : 
              inTree=finalTree              
              outTree ='/gpfs/csic_projects/cms/'+os.getlogin()+'/latino_'+iTarget+'__'+iName+'.root'
        ...
        LepKind = ''
        if 'LepKind' in Productions[iProd] : LepKind = Productions[iProd]['LepKind']
        print 'LepKind : ', LepKind
        command = command.replace('RPLME_LepKind',LepKind)
        ...
        if not 'UEPS' == iStep :
         if 'iihe' in os.uname()[1]:
           ...
           if startingStep == 'Prod' :
             ...
             #print 'cp '+outTree+' '+eosTargBaseOut+'/'+iProd+'/'+startingStep+'__'+iStep+'/latino_'+iTarget+'.root'
             command+='mv '+outTree+' '+eosTargBaseOut+'/'+iProd+'/'+startingStep+'__'+iStep+'/latino_'+iTarget+'.root'
         else:
           if startingStep == 'Prod' :
